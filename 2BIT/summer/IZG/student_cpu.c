...
void phong_onInit(int32_t width,int32_t height){
  ...
  cpu_createBuffers(phong.gpu, 1, &phong.indic);
  cpu_createBuffers(phong.gpu, 1, &phong.vertic);

  cpu_bufferData(phong.gpu, phong.indic, sizeof(bunnyIndices), bunnyIndices);
  cpu_bufferData(phong.gpu, phong.vertic, 1048*sizeof(BunnyVertex), bunnyVertices);

  cpu_createVertexPullers(phong.gpu, 1, &phong.pull);
  cpu_setVertexPullerHead(phong.gpu, phong.pull, 0, phong.vertic,0, 6*(sizeof(float)));
  cpu_setVertexPullerHead(phong.gpu, phong.pull, 1, phong.vertic, 3*(sizeof(float)), 6*(sizeof(float)));
  cpu_enableVertexPullerHead(phong.gpu, phong.pull, 0);
  cpu_enableVertexPullerHead(phong.gpu, phong.pull, 1);

  cpu_setIndexing(phong.gpu, phong.pull, phong.indic, 2);
