...
void gpu_runVertexPuller(
  ...
  int atributes=0;
  while(atributes < MAX_ATTRIBUTES)
  {
  	if ((puller->heads+atributes)->enabled == 1)
            output->attributes[atributes] = gpu_computeVertexAttributeDataPointer(puller->heads+atributes,
                                            gpu_computeGLVertexID(puller->indices,vertexShaderInvocation));
    else
    	output->attributes[atributes] = NULL;
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput atribut;
  primitive->nofUsedVertices = nofPrimitiveVertices;
  unsigned int primitives=0;

  while(primitives < nofPrimitiveVertices)
  {
  	input.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + primitives);
  	gpu_runVertexPuller(&atribut , puller, baseVertexShaderInvocation + primitives);
  	input.attributes=&atribut;
  	vertexShader(primitive->vertices+primitives, &input, gpu);
  	primitives++;
  }
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  sub_Vec2(&vertik1, &vertices[1], &vertices[0]);
  sub_Vec2(&vertik2, &vertices[2], &vertices[0]);
  sub_Vec2(&vertik3, pixelCenter, &vertices[0]);

  float first = dot_Vec2(&vertik2, &vertik2) * dot_Vec2(&vertik3, &vertik1) - dot_Vec2(&vertik1, &vertik2) * dot_Vec2(&vertik3, &vertik2);
  float second = dot_Vec2(&vertik1, &vertik1) * dot_Vec2(&vertik3, &vertik2) - dot_Vec2(&vertik1, &vertik2) * dot_Vec2(&vertik3, &vertik1);
  float men = dot_Vec2(&vertik1, &vertik1) * dot_Vec2(&vertik2, &vertik2) - dot_Vec2(&vertik1, &vertik2) * dot_Vec2(&vertik1, &vertik2);

  coords->data[1]=first/men;
  coords->data[2]=second/men;
